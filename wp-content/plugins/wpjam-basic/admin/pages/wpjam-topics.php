	...
	<ul class="messages">
		...
		<li id="message-<?php echo $message['id']; ?>" class="<?php echo $alternate; echo empty($message['status'])?' unread':'' ?>">
			...
			<div class="message-content">
				<?php echo wpautop(wp_unslash(esc_html($message['content'])));?>
			</div>
		</li>
		<?php } ?>
	</ul>
...
class WPJAM_AdminTopic  {
	...
	public static function get_fields($action_key='', $id=0){
		...
		if($action_key == ''){
			...
			$topic_content	= wpautop(convert_smilies(esc_html($wpjam_topic['content'])));
			...
			if($wpjam_topic['replies']){
				...
				foreach ($wpjam_topic['replies'] as $wpjam_reply) {
					...
					$reply_content	= wpautop(convert_smilies(esc_html($wpjam_reply['content'])));
